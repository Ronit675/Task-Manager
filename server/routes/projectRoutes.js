...
router.post(
  '/',
  asyncHandler(async (request, response) => {
    // Only users with global admin role may create new projects
...
router.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    // Prevent admins from demoting their own membership from the team roster.
    if (email === request.user.email && role === 'MEMBER') {
      response.status(400)
      throw new Error('You cannot change your own role to member')
    }

    // The project creator/owner must always remain an ADMIN.
